...
import { APPS_SCRIPT_URLS, EUNPYEONG_COORDS } from './utils/constants';
...
export default function App() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    const savedTheme = localStorage.getItem('eunpyeong_darkMode');
    return savedTheme !== 'false';
  });

  useEffect(() => {
    localStorage.setItem('eunpyeong_darkMode', isDarkMode.toString());
  }, [isDarkMode]);
  ...
  const [selectedYear, setSelectedYear] = useState(() => {
    return localStorage.getItem('eunpyeong_selectedYear') || currentYear;
  });
  const [selectedMonth, setSelectedMonth] = useState(() => {
    return localStorage.getItem('eunpyeong_selectedMonth') || currentMonth;
  }); 
  const [selectedWeek, setSelectedWeek] = useState(() => {
    return localStorage.getItem('eunpyeong_selectedWeek') || '1주차';
  });

  useEffect(() => {
    localStorage.setItem('eunpyeong_selectedYear', selectedYear);
  }, [selectedYear]);

  useEffect(() => {
    localStorage.setItem('eunpyeong_selectedMonth', selectedMonth);
  }, [selectedMonth]);

  useEffect(() => {
    localStorage.setItem('eunpyeong_selectedWeek', selectedWeek);
  }, [selectedWeek]);
  ...
  const syncWithSupabase = useCallback(async (targetYear = selectedYear, targetMonth = selectedMonth, targetWeek = selectedWeek) => {
    ...
    try {
      ...
      const parsedLocations = validRows.map((row: any[]) => {
        ...
        const lat = EUNPYEONG_COORDS[locName]?.lat || EUNPYEONG_COORDS["기본"].lat;
        const lng = EUNPYEONG_COORDS[locName]?.lng || EUNPYEONG_COORDS["기본"].lng;
